...
const shouldRetry = (errorCount?: ErrorCount, { retryCount = 2, retryTime = 10 * 1000 }: { retryCount?: number; retryTime?: number } = {}) => {
  if (!errorCount) return true;
  return errorCount.count < retryCount || Date.now() > errorCount.last.getTime() + retryTime;
};
...
export const useShowStore = defineStore('data.show', () => {
  ...
  const getShowWatchedProgress = (id: number | string, cacheOptions?: BaseCacheOption) => {
    if (!showsWatchedProgress[id.toString()] && !showWatchedProgressLoading[id.toString()] && shouldRetry(showWatchedProgressError[id.toString()])) {
  ...
  const getShowCollectionProgress = (id: number | string) => {
    if (
      ...
      shouldRetry(showCollectionProgressError[id.toString()])
    ) {
